...
public abstract class Generator<In, Out> {
	 ...
	 * 
	 * @param request
	 *            In object containing client request
	 * @param response
	 *            Out object for the response
	 */
	...
	protected void doDocumentArchive(ProcessState state, String entryName,
			In request, Out response) throws Exception {
	 ...
	 * @throws Exception
	 */
	protected void doSaveReport(ProcessState processState, In request,
			Out response) throws Exception {
	...
	protected void doSaveEntry(ProcessState processState, String entryName,
			In request, Out response) throws IOException, Exception {
	...
	protected boolean doGenerateReport(String entryName, In request,
			Out response) throws Exception, IOException {
	...
	private void doProcessReport(IXDocReport report, String entryName,
			In request, Out response) throws XDocReportException, IOException {
	...
	private void doProcessReportWithConverter(IXDocReport report,
			Options options, In request, Out response)
	 ...
	 * @throws XDocReportException
	 */
	protected IXDocReport getReport(In request) throws IOException,
			XDocReportException {
	...
	protected IXDocReport loadReport(String reportId,
			XDocReportRegistry registry, In request) throws IOException,
			XDocReportException {
		// 2) Get sourceStream
	...
	protected void prepareOptions(Options options, IXDocReport report,
			String converterId, In request) {
	}

	protected boolean isGenerateContentDisposition(String reportId,
			MimeMapping mimeMapping, In request) {
	...
	/**
	 * Returns the template engine to use for the report. By default, it search
	 * if there is template id from request and otherwise returns the default
	 * template engine.
	 * 
	 * @param reportId
	 * @param request
	...
	protected ITemplateEngine getTemplateEngine(IXDocReport report, In request) {
		...
		ITemplateEngine templateEngine = TemplateEngineInitializerRegistry
				.getRegistry().getTemplateEngine(templateEngineKind,
						documentKind);
		if (templateEngine == null) {
			templateEngine = TemplateEngineInitializerRegistry.getRegistry()
					.getTemplateEngine(templateEngineKind, null);
		}
		return templateEngine;
	}

	/**
	 * Returns the template engine from request and otherwise returns the
	...
	protected ITemplateEngine getTemplateEngine(In request) {
		...
		if (StringUtils.isNotEmpty(templateEngineId)) {
			return TemplateEngineInitializerRegistry.getRegistry()
					.getTemplateEngine(templateEngineId);
		}
		return TemplateEngineRegistry.getRegistry().getDefaultTemplateEngine();
	}

	/**
	 * Returns the XDocReport registry which load and cache document. By default
	 * the registry is a singleton. If you wish manage registry per HTTP
	 * session, override this method, create an instance per session and returns
	 * the registry instance linked to the HTTP session.
	 * 
	 * @param request
	 * @return
	 */
	...
	protected abstract void populateContext(IContext context, String reportId,
			In request) throws IOException, XDocReportException;
	...
	protected abstract void prepareHTTPResponse(String id,
			MimeMapping mimeMapping, In request, Out response);

	protected abstract void prepareHTTPResponse(String reportId,
			String entryName, In request, Out response);
