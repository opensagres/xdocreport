...
import fr.opensagres.xdocreport.examples.openoffice.odt.ODTHelloWordWithVelocity;
import fr.opensagres.xdocreport.template.IContext;
import fr.opensagres.xdocreport.template.TemplateEngineKind;

/**
 * Example with Open Office ODT which contains the content Hello !$name. Merge
 * with Velocity template engine will replace this cell with Hello world! and
 * convert it to FO.
 * 
 */
public class ODTHelloWordWithVelocity2FO {
	...
	public static void main(String[] args) {
		try {
			...
			// 4) Merge Java model with the ODT and convert it to FO
			// File out = new File("out");
			// if (!out.exists()) {
			// out.mkdir();
			// }
			// File file = new File(out, "ODTHelloWordWithVelocity.fo");
